...
def process_pdf(pdf_path, file_name, df, link):
    ...
    for i in range(len(doc.ents)):
        ...
        if label == "timexy" or label == "DATE":
            ...
            df = df.append({"file":file_name, "label":label, "page":pg, "text":text, "id":time_id, "surrounding_text":surrounding_text, "link":link + "#page=" + str(pg)}, ignore_index=True)
    print("===========================================================================================================")
...
def process_folder(folder_path, csv_path, reference_dir):
    df = pd.DataFrame(columns=["file", "label", "page", "text", "id", "surrounding_text"])
    reference = pd.read_csv(reference_dir)
    i = 0
    for link in reference["link"]:
        file = str(i) + ".pdf"
        df = process_pdf(folder_path + "/" + file, file, df, link)
        i += 1
    filepath = Path(csv_path)
    filepath.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(filepath, index=False)

process_folder("Data/Depp/Original", "Data/Depp/depp_times.csv", "Data/Depp/depp_witness_statements.csv")
process_folder("Data/Heard/Original", "Data/Heard/heard_times.csv", "Data/Heard/heard_witness_statements.csv")
